...
class DriveService {
    ...
    async listFiles() {
        ...
        try {
            ...
            files.forEach((file) => file.createdTime = moment(file.createdTime));
            if (files.length) {
                logger.info('Files:');
    ...
    async upload(filePath, progressEmitter) {
        const drive = await this.getDriveClient();
    ...
    async downloadFile(fileId, fileName) {
        ...
        return new Promise((resolve, reject) => {
            const filePath = path.join(__dirname, '../../backup/', `downlaod_${fileName}`);
